...
- Gạt phăng ảnh cũ đi, chỉ lấy Text và Insight. AI viết ra một `master_content.md` hoàn toàn mới toanh với lời văn sắc bén của FreeUp! Bài viết này sinh ra và tự lưu vào `media_output/[YYYY-MM-DD]/[Kênh]/[Ticket_ID]/`.
...
- Nếu là **`carousel`**: 
   ...
   - **Mimetics Prompt:** Ra chỉ thị mạnh cho lệnh tạo HTML: "Hãy học theo cách dàn cột (Layout) ở ảnh `layout_snapshot.jpg` nhưng code lại giao diện bằng CSS FreeUp. *CẤM NHÚNG ẢNH CŨ LÊN LÀM NỘI DUNG. Nội dung mới là Typography/Kính FreeUp!*" (Nếu cần không khí, nhúng thẳng thẻ `data-img-vault="image_stock"`. Ngoại lệ duy nhất là chèn ảnh gốc mờ ảo xuống Background).
- Nếu là **`single_image`**:
