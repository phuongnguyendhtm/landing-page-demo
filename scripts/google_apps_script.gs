/**
 * GOOGLE APPS SCRIPT FOR FREEUP CONTENT MACHINE V3 (HYBRID SYNC)
...
function doGet(e) {
  ...
  if (action === "get_publish_tasks") {
    try {
      const sheet = ss.getSheetByName(TAB_PIPELINE);
      if (!sheet) return ContentService.createTextOutput(JSON.stringify({error: "Không tìm thấy tab " + TAB_PIPELINE})).setMimeType(ContentService.MimeType.JSON);
      
      const data = sheet.getDataRange().getValues();
      const tasks = [];
      
      for (let i = 1; i < data.length; i++) {
        if (data[i][6] === "🚀 ĐĂNG BÀI NGAY") { // Cột G (Publish)
          tasks.push({
            row: i + 1,
            ticketId: data[i][0],
            format: data[i][1],
