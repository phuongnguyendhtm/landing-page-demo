 ...
 * Business Growth Marketing - Bridge between Local and Google Sheets
...
const DATABASE_PATHS = {
  ...
  inventory: path.join(__dirname, '..', 'database', 'post_inventory.json')
};
...
/**
 * 3.1. Kéo lệnh Đăng bài từ Cloud về máy (Tab Content Pipeline -> Local Inventory)
 */
async function syncPublishTasksDown() {
  console.log('🔄 Đang kiểm tra lệnh ĐĂNG BÀI từ Google Sheets...');
  try {
    const response = await axios.get(`${GOOGLE_SHEET_APP_URL}?action=get_publish_tasks`);
    const publishTasks = response.data;

    if (!Array.isArray(publishTasks) || publishTasks.length === 0) {
      console.log('ℹ️ Không có lệnh đăng bài mới nào được kích hoạt trên Cloud.');
      return;
    }

    const inventory = JSON.parse(fs.readFileSync(DATABASE_PATHS.inventory, 'utf8'));
    let updatedCount = 0;

    for (const task of publishTasks) {
      // Tìm bài đăng này trong inventory
      let post = inventory.find(p => p.post_id === task.ticketId);
      
      if (!post) {
        // Nếu không có (có thể do tạo thủ công trên sheet), tạo mới record trong inventory
        post = {
          post_id: task.ticketId,
          source_idea_id: "from_cloud",
          delivery_format: task.format.toLowerCase().includes('reels') ? 'reels' : (task.format.toLowerCase().includes('broll') ? 'broll' : 'image'),
          target_channels: [task.channel || "facebook"],
          media_link: task.mediaPath,
          caption_link: task.mediaPath.replace(/[^\\/]+$/, 'caption.txt'), // Giả định path caption
          status: "pending_publish",
          created_at: new Date().toISOString()
        };
        inventory.push(post);
        updatedCount++;
      } else if (post.status !== "published" && post.status !== "pending_publish") {
    ...
    if (updatedCount > 0) {
      fs.writeFileSync(DATABASE_PATHS.inventory, JSON.stringify(inventory, null, 2));
      console.log(`✅ Đã chuẩn bị ${updatedCount} bài đăng sẵn sàng kích hoạt Playwright.`);
    }
    
    return publishTasks.map(t => t.ticketId);
  } catch (error) {
    console.error('❌ Lỗi syncPublishTasksDown:', error.message);
    return [];
  }
}
